...
class SSHLibrary(object):
    ...
    def execute_command(self, command, return_stdout=True, return_stderr=False,
                        return_rc=False, log_level=True):
        ...
        return self._return_command_output(stdout, stderr, rc, log_level=False, *opts)

    #private function ---start
    def execute_command_as_user(self, user, command, return_stdout=True, return_stderr=False,
                        return_rc=False, log_level=True):
        """Executes `command` on the remote machine and returns its outputs.
        ...
        """
        if log_level:
            self._info("Executing command '%1s' as user '%2s'." % (command, user))
        opts = self._legacy_output_options(return_stdout, return_stderr,
                                           return_rc)
        new_command = "su %1s -c '%2s'" % (user, command)
        stdout, stderr, rc = self.current.execute_command(new_command)
        return self._return_command_output(stdout, stderr, rc, log_level=False, *opts)
    ...
    def _return_command_output(self, stdout, stderr, rc, return_stdout,
                               return_stderr, return_rc, log_level=True):
        if log_level:
    ...
    def wait_until_command_has_ouput(self, command, timeout, error = None, loglevel=None):
        """Wait until the command has output. If it has no output in TIMEOUT, it will raise error."""

        def check_output():
            is_output = self.execute_command(command, log_level=False)
            if is_output:
                return
            else:
                return error or "Command has no ouput in %s" % self._format_timeout(timeout)

        self._wait_util_no_error(timeout, check_output)

    def _wait_util_no_error(self, timeout, wait_function, *args):

        timeout = robot.utils.timestr_to_secs(timeout)
        max_time = time.time() + timeout
        while True:
            timeout_error = wait_function(*args)
